...
import { Row, Col, Card, Form, Input, Select, Button, Menu, message } from 'antd';
import LogsTable from '../logtable/index';
...
class LogsList extends PureComponent {
  ...
  fetchTableList = (params = {}) => {
    ...
    let REQUEST_URL = `/api/admin/log/listLogs?${stringify(params)}`;
...
removeUser = ()=>{
  ...
  const params={
		  logIds:selectedRows.map(row => row.id).join(','),
  };
  ...
  request(`/api/admin/log/removeLogs`,newOptions,this.c2)
}
c2=(data)=>{
	   this.setState({
		      selectedRows: [],
